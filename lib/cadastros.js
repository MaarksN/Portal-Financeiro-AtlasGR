...
function criarCliente(dados, atorEmail) {
  ...
  const id = executar(
    `INSERT INTO clientes (documento, nome, email, telefone)
     VALUES (?, ?, ?, ?)`,
...
function listarProdutos(apenasAtivos = false) {
  ...
  return consultar(`SELECT * FROM produtos ${where} ORDER BY nome`);
}

function criarProduto(dados, atorEmail) {
  if (dados.codigo) {
    const existente = consultarUm('SELECT id FROM produtos WHERE codigo = ?', dados.codigo);
    if (existente) throw new ErroApp('Já existe um produto com este código.', { status: 409, codigo: 'conflito' });
  }

  const id = executar(
    `INSERT INTO produtos (codigo, nome, descricao, preco_centavos, ativo)
     VALUES (?, ?, ?, ?, ?)`,
    dados.codigo || null, dados.nome, dados.descricao || null, dados.preco_centavos || 0, dados.ativo ?? 1
  ).lastInsertRowid;
...
function listarServicos(apenasAtivos = false) {
  const where = apenasAtivos ? 'WHERE ativo = 1' : '';
  return consultar(`SELECT * FROM servicos ${where} ORDER BY nome`);
}

function criarServico(dados, atorEmail) {
  if (dados.codigo) {
    const existente = consultarUm('SELECT id FROM servicos WHERE codigo = ?', dados.codigo);
    if (existente) throw new ErroApp('Já existe um serviço com este código.', { status: 409, codigo: 'conflito' });
  }

  const id = executar(
    `INSERT INTO servicos (codigo, nome, descricao, preco_centavos, ativo)
     VALUES (?, ?, ?, ?, ?)`,
    dados.codigo || null, dados.nome, dados.descricao || null, dados.preco_centavos || 0, dados.ativo ?? 1
  ).lastInsertRowid;
