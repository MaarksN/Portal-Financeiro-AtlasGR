...
function vincularEmpresa(usuarioId, empresaId, atorEmail) {
  const existente = consultarUm('SELECT * FROM usuarios_empresas WHERE usuario_id = ? AND empresa_id = ?', usuarioId, empresaId);
  if (!existente) {
    executar('INSERT INTO usuarios_empresas (usuario_id, empresa_id) VALUES (?, ?)', usuarioId, empresaId);
    auditoria.registrar({ ator: atorEmail, acao: 'usuario.vinculado_empresa', entidade: 'usuario', entidadeId: usuarioId, detalhe: { empresaId } });
  }
}

function listarEmpresasVinculadas(usuarioId) {
  return consultar('SELECT e.* FROM empresas e JOIN usuarios_empresas ue ON e.id = ue.empresa_id WHERE ue.usuario_id = ? ORDER BY e.razao_social', usuarioId);
}
